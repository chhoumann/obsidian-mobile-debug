...
- `android_waypoint_validate.py` — instruments the vault event bus (`vault.on('create'|'modify'|...)`), fires one download, and reports crash / byte-perfection / leftover temp files / index-resolvability. Built to prove the PodNotes×Waypoint crash and its fix; run it across the matrix (old vs new build, watcher plugin off vs on).

What the Android trace settled (2026-06): in a **clean** vault the streamed download is memory-clean on Android — `appendBinary` is O(1), JS heap stays flat (~22 MB) through a 47–155 MB download, no crash — and chunk size barely affects wall-time (connection reuse amortizes the redirect chains), so it is not a slowness lever. That clean-vault result is also what made the real bug *hard* to see: the reported crash only reproduces with a **vault-watcher plugin installed**. Appending each chunk to the final vault path fires ~1 create + ~11 modify events on the growing file; Waypoint (and any `vault.on('modify')` plugin — Dataview, Obsidian Git) calls `cachedRead` on each event with no extension filter, loading the growing media file into memory over and over → OOM. PodNotes 2.17.2 fixes it by streaming to a dot-prefixed temp (which Obsidian keeps out of the file index, so it fires zero watcher events) then `adapter.rename`-ing it into place (one create of the finished file). `android_waypoint_validate.py` measured the event count drop from ~12 to 1 and the crash disappearing with Waypoint enabled.
